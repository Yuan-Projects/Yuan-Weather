...
describe('HeWeather API Tests', function() {
  ...
  it('[setup]This project is properly configured', () => {
    ...
    assert.ok(config.WUNDERGROUND_API, 'wunderground API key has been set up')
  });

  it('#1 Get weather infomation in Yantian District, Shenzhen, China', function(done) {
    dataProvider.getWeatherDataByLocation('yantian').then(function(data) {
      assert.equal(data.location.country_name, '中国');
      assert.equal(data.location.state, '广东');
      assert.equal(data.location.city, '深圳');

      assert.ok(data.current_observation.temperature, "current temperature returned.");
      assert.ok(data.current_observation.weather, "current weather returned.");
      assert.ok(data.current_observation.icon_url, "current icon_url returned.");
      assert.ok(data.current_observation.wind, "current wind returned.");
      assert.ok(data.current_observation.observation_time, "current observation_time returned.");
  ...
  let perthFunc = function(done, query) {
    dataProvider.getWeatherDataByLocation(query).then(function(data) {
  ...
  it('#1 Get weather infomation in Perth, Western Australia by geolocation', (done) => {
    ...
    perthFunc(done, '115.8621,-31.9674');
  });
